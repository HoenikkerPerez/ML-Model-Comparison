...
class MultioutputLineamModel(BaseEstimator, RegressorMixin):
    """
    ...
    def fit(self, X_train, y_train):
        self.lm0.fit(X_train, y_train[:, 0])
        self.lm1.fit(X_train, y_train[:, 1])
        return self

    def predict(self, X_test):
        output = np.zeros(shape=(X_test.shape[0], self.outdim))
        output[:, 0] = self.lm0.predict(X_test)
        output[:, 1] = self.lm1.predict(X_test)
        return output
...
def LASSO_model_selection(X_train, y_train, mode="regression"):
    ...
    if mode == "regression":
        ...
        tuned_parameters = {'reg__alpha': alpha_range}
        ...
        scorer = make_scorer(mean_euclidian_error_loss, greater_is_better=False)
        #         # gs = GridSearchCV(estimator=pipe_svr, cv=5, param_grid=tuned_parameters, scoring=scorer, verbose=1, n_jobs=-1)
        gs1 = GridSearchCV(estimator=pipe_svr,
                          cv=5,
                          param_grid=tuned_parameters,
                          scoring=scorer,
                          # verbose=1,
        ...
        return MultioutputLineamModel(gs0.best_estimator_, gs1.best_estimator_)
