...
def svr_model_selection(X_train, y_train):
    ...
    plot_search_heatmap(coarse_gs, "SVR Coarse Gridsearch")
    plot_search_results(coarse_gs, "SVR Coarse parameters")
    C, gamma, eps = coarse_gs.best_params_['reg__base_estimator__C'], coarse_gs.best_params_['reg__base_estimator__gamma']
    # C, gamma = coarse_gs.best_params_['C'], coarse_gs.best_params_['gamma']
    plot_search_results(coarse_gs, "SVC parameters", vmin=0, vmax=1)

    finer_gs = svr_gridsearch(X_train, y_train, is_fine_search=True, coarse_gamma=gamma, coarse_C=C, coarse_eps=eps)
...
def svr_gridsearch(X_train, y_train, is_fine_search=False, coarse_gamma=None, coarse_C=None, coarse_eps=.1):
    if DEBUG:
        ...
        if is_fine_search:
            gamma_range = np.round(np.linspace(coarse_gamma / 5., coarse_gamma * 5, num=25),2)
            C_range = np.round(np.linspace(coarse_C / 5., coarse_C * 5., num=25),2)
            # epsilon_range = [coarse_eps]
    ...
    tuned_parameters = {'reg__base_estimator__gamma': gamma_range,
                        # 'reg__base_estimator__epsilon': epsilon_range,
